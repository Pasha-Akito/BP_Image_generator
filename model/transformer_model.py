...
class TextToImageTransformer(nn.Module):
    def __init__(
        ...
        single_encoder_layer = nn.TransformerEncoderLayer(
            d_model=embedding_dimensions, nhead=total_attention_heads, batch_first=True
        )
        self.transformer_encoder = nn.TransformerEncoder(
            single_encoder_layer, num_layers=total_encoder_layers
        )
    ...
    def forward(self, text):
        ...
        context_vector = torch.sum(
            transformer_output * attention_weights.unsqueeze(-1), dim=1
        )
